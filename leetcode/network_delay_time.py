# Medium: 743
# Find the maximum time it takes to any node from node K.
# If a node can not be reached from K, return -1.
# Dijkstra algorithm
...
class Solution:
    def networkDelayTime(self, times: List[List[int]], N: int, K: int) -> int:
        ...
        max_time = max(dist[1:])
        return max_time if max_time < max_value else -1

    def networkDelayTime_dijkstra(self, times: List[List[int]], N: int, K: int) -> int:
        time_dict:DefaultDict[int, List] = defaultdict(list)
        visited = [False]*(N+1)
        queue: List[Tuple] = list()
        max_value = 1000000
        dist: List[int] = [max_value]*(N+1)

        for edge in times:
            time_dict[edge[0]].append(edge)
        ...
        while len(queue)>0:
            ...
            for nei in time_dict[current]:
                if not visited[nei[1]]:
                    heapq.heappush(queue, (d+nei[2], nei[1]))
